...
{
    char http_request[MAXREQUESTSIZE];
    int total_nbytes, last_nbytes_received;
    char get_request_line[MAXREQUESTSIZE];
    char host_name_line[MAXREQUESTSIZE];
    char* line_from_request;
    ...
    {
        last_nbytes_received = read(filedes, http_request, MAXREQUESTSIZE);
    ...
    if (total_nbytes > 0)
    {
        ...
        /* first step: initialize the strings that will hold the importnt
           fields to be empty */
        strcpy(get_request_line, "");
        strcpy(host_name_line, "");

        /* second step: get first line as the first line of HTTP request header
         is always the request-line */
        line_from_request = strtok(http_request, "\n");
        strcpy(get_request_line, line_from_request);
        ...
        {
            if (strstr(line_from_request, "Host: ")) strcpy(host_name_line, line_from_request + 6);
            line_from_request = strtok(NULL, "\n");
        }

        printf("the get_request_line was %s \nthe host_name was %s\n", get_request_line, host_name_line);
        fflush(stdout);
